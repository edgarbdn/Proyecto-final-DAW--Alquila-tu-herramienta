...
const CARD_STYLES: Record<string, { border: string; bg: string }> = {
  ...
  finalizado: { border: "border-gray-200",   bg: "bg-white" },
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtrados.map((a) => {
            ...
            return (
              <div key={a.id} className={`rounded-2xl border shadow-sm overflow-hidden ${CARD_STYLES[a.estado]?.border ?? "border-gray-200"} ${CARD_STYLES[a.estado]?.bg ?? "bg-white"}`}>
