...
export async function GET(request: NextRequest) {
  ...
  if (!user) return NextResponse.json({ error: ERROR_MESSAGES.NO_AUTENTICADO }, { status: 401 });
  ...
  if (error) return apiError("GET /recibidos", error, 500, ERROR_MESSAGES.ERROR_SERVIDOR);
...
export async function PATCH(request: NextRequest) {
  ...
  if (!alquiler) return NextResponse.json({ error: ERROR_MESSAGES.NO_ENCONTRADO }, { status: 404 });
  ...
  if (error) return apiError("PATCH /recibidos", error, 500, ERROR_MESSAGES.ERROR_SERVIDOR);
